...
class RevokeMessage extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _RevokeMessageState();
  }
}

class _RevokeMessageState extends State<RevokeMessage> {
  String result = '';
  List<String> friends = [];
  List<String> groups = [];
  ...
  pressedFunction() async {
    V2TimCallback revokeMessageRes =
        await TencentImSDKPlugin.v2TIMManager.getMessageManager().revokeMessage(
              msgID: selectedMessage.msgID!, // 需要撤回的消息id
            );
    setState(() {
      result = revokeMessageRes.toJson().toString();
    });
  }
