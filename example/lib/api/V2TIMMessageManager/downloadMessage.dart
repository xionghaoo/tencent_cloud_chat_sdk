...
class DownloadMessage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return _DownloadMessageState();
  }
}

class _DownloadMessageState extends State<DownloadMessage> {
  String result = '';
  List<String> friends = [];
  List<String> groups = [];
    ...
    if (selectedId.isNotEmpty) {
          ...
          await TencentImSDKPlugin.v2TIMManager
              ...
              .getHistoryMessageList(
                getType: HistoryMsgGetTypeEnum
                    .V2TIM_GET_CLOUD_OLDER_MSG, // 拉取消息的位置及方向
                userID:
      ...
      if (getHistoryMessageListRes.code == 0) {
        ...
        getHistoryMessageListRes.data?.forEach((element) {
          if (element.elemType == MessageElemType.V2TIM_ELEM_TYPE_IMAGE) {
            temp.add(element);
          }
      ...
      print(getHistoryMessageListRes.toJson());
      if (getHistoryMessageListRes.code == 0) {
        List<V2TimMessage> temp = [];
        getHistoryMessageListRes.data?.forEach((element) {
          if (element.elemType == MessageElemType.V2TIM_ELEM_TYPE_IMAGE) {
  ...
  pressedFunction() async {
    int imageType =
        selectedMessage.elemType == MessageElemType.V2TIM_ELEM_TYPE_IMAGE
            ? 2
            : 0;
    ...
    V2TimCallback downloadMessageRes = await TencentImSDKPlugin.v2TIMManager
        .getMessageManager()
        .downloadMessage(
            msgID: selectedMessage.msgID!, // 消息id
            messageType: selectedMessage.elemType, // 消息类型
            imageType: imageType, // 图片类型，仅messageType为图片消息是有效
            isSnapshot: isSnapshot // 是否是视频封面，仅messageType为视频消息是有效
            );
    setState(() {
      result = downloadMessageRes.toJson().toString();
    });
  }
