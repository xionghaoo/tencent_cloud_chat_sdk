...
class SetGroupMemberInfo extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _SetGroupMemberInfoState();
  }
}

class _SetGroupMemberInfoState extends State<SetGroupMemberInfo> {
  String result = '';
  List<String> friends = [];
  List<String> groups = [];
  String selectedId = '';
  ...
  pressedFunction() async {
    V2TimCallback setGroupMemberInfoRes = await TencentImSDKPlugin.v2TIMManager
        .getGroupManager()
        .setGroupMemberInfo(
          groupID: selectedGroup, // 需要修改的群组id
          userID: selectedId, // 需要修改的用户id
          nameCard: _statusController.text, // 修改名片属性
        );
    setState(() {
      result = setGroupMemberInfoRes.toJson().toString();
    });
  }
