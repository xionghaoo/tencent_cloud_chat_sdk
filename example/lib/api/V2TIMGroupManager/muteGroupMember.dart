...
class MuteGroupMember extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _MuteGroupMemberState();
  }
}

class _MuteGroupMemberState extends State<MuteGroupMember> {
  String result = '';
  List<String> friends = [];
  List<String> groups = [];
  String selectedId = '';
  ...
  pressedFunction() async {
    V2TimCallback muteGroupMemberRes =
        await TencentImSDKPlugin.v2TIMManager.getGroupManager().muteGroupMember(
            groupID: selectedGroup, // 禁言的群组id
            userID: selectedId, // 禁言的用户id
            seconds: 60 // 禁言时间
            );
    setState(() {
      result = muteGroupMemberRes.toJson().toString();
    });
  }
