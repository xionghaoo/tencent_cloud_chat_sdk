...
class GetGroupMembersInfo extends StatefulWidget {
  ...
  State<StatefulWidget> createState() {
    return _GetGroupMembersInfoState();
  }
}

class _GetGroupMembersInfoState extends State<GetGroupMembersInfo> {
  String result = '';
  List<String> friends = [];
  List<String> groups = [];
  String selectedId = '';
  ...
  pressedFunction() async {
    V2TimValueCallback<List<V2TimGroupMemberFullInfo>> getGroupMembersInfoRes =
        await TencentImSDKPlugin.v2TIMManager
            .getGroupManager()
            .getGroupMembersInfo(
      groupID: selectedGroup, // 需要获取的群组id
      memberList: [selectedId], // 需要获取的用户id列表
    );
    setState(() {
      result = getGroupMembersInfoRes.toJson().toString();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      appBar: AppBar(
        ...
        title: Text('GetGroupMembersInfo'),
      ),
      body: Center(
        child: Column(
          children: [
