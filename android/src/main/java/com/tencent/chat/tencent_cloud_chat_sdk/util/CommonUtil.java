...
//            IMLog.i("tencent_im_sdk_plugin_res",map.toString());
            CommonUtil.writeLog(map.toString(),true);
        }catch (Exception e){
    ...
    public static <T> void returnSuccess(final MethodChannel.Result result,T data) {
        final HashMap<String,Object> succ = new HashMap<String,Object>();
        succ.put("code",0);
        succ.put("desc","ok");
        succ.put("data",data);
        result.success(succ);
    ...
    public static <T> void emitEvent(final MethodChannel channel, final String method, String type, T data, String listenerUuid){
    ...
    public static HashMap<String,Object> convertV2TIMGroupMemberInfoToMap(V2TIMGroupMemberInfo info){
        ...
        ginfo.put("onlineDevices ",info.getOnlineDevices());
    ...
    public static HashMap<String,Object> convertV2TIMMessageToMap(V2TIMMessage msg,Object ...progress){
        ...
        message.put("isSupportMessageExtension",msg.isSupportMessageExtension());
        message.put("needReadReceipt", msg.isNeedReadReceipt());

        message.put("hasRiskContent",msg.hasRiskContent());
        message.put("revokeReason",msg.getRevokeReason());
        message.put("isBroadcastMessage",msg.isBroadcastMessage());
        if(msg.getRevokerInfo() != null){
            message.put("revokerInfo",CommonUtil.convertV2TIMUserFullInfoToMap(msg.getRevokerInfo()));
        }else{
            message.put("revokerInfo",null);
        }

        message.put("isExcludedFromContentModeration",msg.isExcludedFromContentModeration());
        ...
        if(type == V2TIMMessage.V2TIM_ELEM_TYPE_TEXT){
            ...
            final HashMap<String,Object> sound = CommonUtil.convertSoundMessageElem(soundElem,false);
            if(nextElem != null) {
            ...
            final HashMap<String,Object> video = CommonUtil.convertVideoMessageElem(videoElem,false);
            if(nextElem != null) {
            ...
            final HashMap<String,Object> file = CommonUtil.convertFileMessageElem(fileElem,false);
            if(nextElem != null) {
    ...
    public static void downloadImageMessage(final String msgID, V2TIMImageElem elem, int imageType,final DownloadCallback callback){
        ...
        final String downloadKey = msgID + ("_0");
        for (int i=0;i<elem.getImageList().size();i++){
            ...
            try{
                if(cacheDir.exists()){
                    final String path =  cacheDir.getPath()+"/image_"+imgInstance.getType()+""+imgInstance.getSize()+""+imgInstance.getWidth()+imgInstance.getHeight()+"_temp_"+imgInstance.getUUID();
                    final String finalPath = cacheDir.getPath()+"/image_"+imgInstance.getType()+""+imgInstance.getSize()+""+imgInstance.getWidth()+imgInstance.getHeight()+"_"+imgInstance.getUUID();
                    final File finalFile = new File(finalPath);
                    ...
                    if(!finalFile.exists()){
                        imgInstance.downloadImage(path, new V2TIMDownloadCallback() {
                            ...
                            public void onProgress(V2TIMElem.V2ProgressInfo v2ProgressInfo) {
                                ...
                                callback.onProgress(false,false,v2ProgressInfo.getCurrentSize(),v2ProgressInfo.getTotalSize(),msgID,imgInstance.getType(),false,finalPath,0,"");
                            }
                            ...
                            public void onSuccess() {
                                ...
                                MessageManager.downloadingMessageList.remove(downloadKey);
                                callback.onProgress(true,false,0,0,msgID,imgInstance.getType(),false,finalPath,0,"");
                            }
                            ...
                            public void onError(int i, String s) {
                                ...
                                MessageManager.downloadingMessageList.remove(downloadKey);
                                tempFile.delete();
                                callback.onProgress(false,false,0,0,msgID,imgInstance.getType(),false,finalPath,i,s);
                            }
                        });
                    }else{
    ...
    public static void downloadFileMessage(final String msgID, V2TIMFileElem elem, final DownloadCallback callback){
        ...
        try {
            ...
            if (cacheDir.exists()) {
                ...
                final String path = cacheDir.getPath() + "/file_"+ elem.getUUID()+(elem.getUUID().contains(".")?"":"_temp_"+URLEncoder.encode(elem.getFileName(), "UTF-8"));
                final String finalPath = cacheDir.getPath() + "/file_"+ elem.getUUID()+(elem.getUUID().contains(".")?"":"_"+URLEncoder.encode(elem.getFileName(), "UTF-8"));
                final File tempFile = new File(path);
                final File finalFile = new File(finalPath);
                if(!finalFile.exists()){
                    elem.downloadFile(path, new V2TIMDownloadCallback() {
                        @Override
                        public void onProgress(V2TIMElem.V2ProgressInfo v2ProgressInfo) {
                            callback.onProgress(false,false,v2ProgressInfo.getCurrentSize(),v2ProgressInfo.getTotalSize(),msgID,0,false,finalPath,0,"");
                        }
                        ...
                        public void onError(int i, String s) {
                            ...
                            tempFile.delete();
                            MessageManager.downloadingMessageList.remove(downloadKey);
                            callback.onProgress(false,true,0,0,msgID,0,false,finalPath,i,s);
                        }
                    });
                }else{
                    MessageManager.downloadingMessageList.remove(downloadKey);
    ...
    public static void downloadDoundMessage(final String msgID, V2TIMSoundElem elem, final DownloadCallback callback){
        ...
        try {
            ...
            File cacheDir = context.getFilesDir();
            if (cacheDir.exists()) {
                final String path = cacheDir.getPath() + "/sound_temp"+ elem.getUUID();
                final String finalPath = cacheDir.getPath() + "/sound_"+ elem.getUUID();
    ...
    public static void downloadVideoMessage(final String msgID, V2TIMVideoElem elem,boolean isSnapshot, final DownloadCallback callback){
        ...
        try {
            ...
            if (cacheDir.exists()) {
                final String path = cacheDir.getPath() + "/video_temp_"+ elem.getVideoUUID();
                final String snipPath = cacheDir.getPath() + "/video_temp_" + elem.getSnapshotUUID();
                final String finalPath = cacheDir.getPath() + "/video_"+ elem.getVideoUUID();
                final String finalSnipPath = cacheDir.getPath() + "/video_" + elem.getSnapshotUUID();
                final File file = new File(path);
                final File snip = new File(snipPath);
                final File finalFile = new File(finalPath);
                final File finalSnip = new File(finalSnipPath);
                if(!isSnapshot){
                    if (!finalFile.exists()) {
                    ...
                    if(!finalSnip.exists()){
                        elem.downloadSnapshot(snipPath, new V2TIMDownloadCallback() {
                            @Override
                            public void onProgress(V2TIMElem.V2ProgressInfo v2ProgressInfo) {
                                callback.onProgress(false,false,v2ProgressInfo.getCurrentSize(),v2ProgressInfo.getTotalSize(),msgID,0,true,finalSnipPath,0,"");
                            }

                            @Override
                            public void onSuccess() {
                                snip.renameTo(finalSnip);
                                MessageManager.downloadingMessageList.remove(downloadKey);
                                callback.onProgress(true,false,0,0,msgID,0,true,finalSnipPath,0,"");
                            }
                            ...
                            public void onError(int i, String s) {
                                ...
                                MessageManager.downloadingMessageList.remove(downloadKey);
                                System.out.println("下载失败:"+i+","+s);
                                callback.onProgress(false,true,0,0,msgID,0,true,finalSnipPath,i,s);
                            }
                        });
                    }else{
                        MessageManager.downloadingMessageList.remove(downloadKey);
                        callback.onProgress(true,false,0,0,msgID,0,false,finalSnipPath,0,"");
                    }
                }
    ...
    public static  HashMap<String,Object> convertImageMessageElem(V2TIMImageElem elem) {
        ...
        for (int i=0;i<elem.getImageList().size();i++){
            ...
            try {
                File cacheDir =  context.getExternalCacheDir();
                File latestcacheDir = context.getFilesDir();
                if(latestcacheDir.exists()){
                    String path =  latestcacheDir.getPath()+"/image_"+imgInstance.getType()+""+imgInstance.getSize()+""+imgInstance.getWidth()+imgInstance.getHeight()+"_"+imgInstance.getUUID();
                    File file = new File(path);
                    if(file.exists()){
                        item.put("localUrl",path);
                    }
                }
                if(cacheDir.exists() && !item.containsKey("localUrl")){
    ...
    public static  HashMap<String,Object> convertSoundMessageElem(V2TIMSoundElem soundElem,boolean isGetUrl) {
        ...
        try {
            ...
            File latestcacheDir = context.getFilesDir();
            if(latestcacheDir.exists()){
                String path = latestcacheDir.getPath() + "/sound_" + soundElem.getUUID();
                File file = new File(path);
                if(file.exists()){
                    sound.put("localUrl",path);
                }
            }
            if (cacheDir.exists() && !sound.containsKey("localUrl")) {
    ...
    public static  HashMap<String,Object> convertVideoMessageElem(V2TIMVideoElem videoElem,boolean isGetUrl) {
        ...
        try {
            ...
            File latestcacheDir = context.getFilesDir();
            if(latestcacheDir.exists()){
                String path = latestcacheDir.getPath() + "/video_" + videoElem.getVideoUUID();
                String snipPath = latestcacheDir.getPath() + "/video_" + videoElem.getSnapshotUUID();
                File file = new File(path);
            ...
            if (cacheDir.exists()) {
                if(!video.containsKey("localVideoUrl")){
                ...
                if(!video.containsKey("localSnapshotUrl")){
    ...
    public static  HashMap<String,Object> convertFileMessageElem(V2TIMFileElem fileElem,boolean isGetUrl) {
        ...
        try {
            ...
            File latestcacheDir = context.getFilesDir();
            if(latestcacheDir.exists()){
                final String finalPath = latestcacheDir.getPath() + "/file_"+ fileElem.getUUID()+(fileElem.getUUID().contains(".")?"":"_"+URLEncoder.encode(fileElem.getFileName(), "UTF-8"));
                File f = new File(finalPath);
                if(f.exists()){
                    file.put("localUrl",finalPath);
                }
            }
            if (cacheDir.exists() && !file.containsKey("localUrl")) {
                
                final String finalPath = cacheDir.getPath() + "/file_"+ fileElem.getUUID()+(fileElem.getUUID().contains(".")?"":"_"+URLEncoder.encode(fileElem.getFileName(), "UTF-8"));
                File f = new File(finalPath);
                if(f.exists()){
                    file.put("localUrl",finalPath);
                }
    ...
    public static HashMap<String,Object> convertMessageElem(V2TIMElem elem) {
        ...
        if (elem != null) {
            if(elem instanceof V2TIMCustomElem) {
                ...
                messageElem = CommonUtil.convertSoundMessageElem((V2TIMSoundElem) elem,false);
                messageElem.put("elemType", V2TIMMessage.V2TIM_ELEM_TYPE_SOUND);
            } else if (elem instanceof  V2TIMVideoElem) {
                messageElem = CommonUtil.convertVideoMessageElem((V2TIMVideoElem) elem,false);
                messageElem.put("elemType", V2TIMMessage.V2TIM_ELEM_TYPE_VIDEO);
            } else if (elem instanceof  V2TIMFileElem) {
                messageElem = CommonUtil.convertFileMessageElem((V2TIMFileElem) elem,false);
    ...
    public static HashMap<String,Object> convertV2TIMGroupChangeInfoToMap(V2TIMGroupChangeInfo info){
        ...
        rinfo.put("intValue",info.getIntValue());
        return rinfo;
    }
    public static HashMap<String,Object> convertV2TIMReceiveMessageOptInfoToMap(V2TIMReceiveMessageOptInfo info){
        ...
        rinfo.put("allReceiveMessageOpt",info.getAllReceiveMessageOpt());
        rinfo.put("duration",info.getDuration());
        rinfo.put("startHour",info.getStartHour());
        rinfo.put("startSecond",info.getStartSecond());
        rinfo.put("startTimeStamp",info.getStartTimeStamp());
        rinfo.put("startMinute",info.getStartMinute());
    ...
    public static HashMap<String,Object> convertV2TIMMessageReceiptToMap(V2TIMMessageReceipt info){
        ...
        rinfo.put("isPeerRead",info.isPeerRead());
        return rinfo;
    }
    ...
    public static List<HashMap<String,Object>> convertV2TIMGroupMemberFullInfoListToMap(List<V2TIMGroupMemberFullInfo> infos){
        LinkedList<HashMap<String,Object>> rli = new LinkedList<>();
        for (V2TIMGroupMemberFullInfo info : infos) {
            rli.add(CommonUtil.convertV2TIMGroupMemberFullInfoToMap(info));
        }
        return  rli;
    }
    public static HashMap<String,Object> convertV2TIMGroupMemberFullInfoToMap(V2TIMGroupMemberFullInfo info){
        ...
        rinfo.put("isOnline",info.isOnline());
        rinfo.put("onlineDevices",info.getOnlineDevices());
        return rinfo;
    }
    ...
    public static HashMap<String,Object> convertV2TimGroupApplicationToMap(V2TIMGroupApplication info){
        ...
        rinfo.put("type",info.getApplicationType());

        return rinfo;
    }
    public static boolean isValidateTrace(String t){
        boolean trace = false;
        if(!t.isEmpty() && t.contains("TencentImSDKPlugin")){
            trace = true;
        }
        return trace;
    }
    public static List<String> checkApiContains(String t){
        List<String> list = new LinkedList<>(Arrays.asList("tencentAVChatRoom","tencentChatVote"));
        List<String> resList = new LinkedList<>();
        for (int i = 0; i < list.size(); i++) {
            if(t.contains(list.get(i))){
                resList.add(""+i);
            }
        }
        return resList;
    }
    public static LinkedList<String> getCheckList(String i){
        HashMap<String,LinkedList<String>> resMap = new HashMap<>();
        LinkedList<String> avList = new LinkedList<>();
        resMap.put("0",avList);
        LinkedList<String> voteList = new LinkedList<>(Arrays.asList("createCustomMessage", "setMessageExtensions", "sendMessage","getMessageExtensions","getGroupsInfo","getGroupMembersInfo"));
        resMap.put("1",voteList);
        LinkedList<String> solitaireList = new LinkedList<>();
        resMap.put("2",solitaireList);
        LinkedList<String> redPackList = new LinkedList<>();
        resMap.put("3",redPackList);
        return  resMap.get(i);
    }
    ...
    public static HashMap<String,Object> convertV2TIMConversationToMap(V2TIMConversation info){
        ...
        rinfo.put("c2cReadTimestamp",info.getC2CReadTimestamp());
        rinfo.put("groupReadSequence",info.getGroupReadSequence());
        List<Long> mli = new LinkedList<>();
        for(int i = 0;i<info.getMarkList().size();i++){
            mli.add(info.getMarkList().get(i).longValue());
        }
    ...
    public static HashMap<String,Object> convertV2TIMUserInfoToMap(V2TIMUserInfo info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();

        rinfo.put("userID",info.getUserID());
        rinfo.put("faceUrl",info.getFaceUrl());
        rinfo.put("nickName",info.getNickName());
        rinfo.put("birthday",info.getBirthday());
        return  rinfo;
    }
    public static HashMap<String,Object> convertV2TIMMessageReactionToMap(V2TIMMessageReaction info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("reactionID",info.getReactionID());
        rinfo.put("reactedByMyself",info.getReactedByMyself());
        rinfo.put("totalUserCount",info.getTotalUserCount());
        List<V2TIMUserInfo> list = info.getPartialUserList();
        LinkedList<Object> clist = new LinkedList<Object>();
        for(int i = 0;i<list.size();i++){
            V2TIMUserInfo item = list.get(i);
            HashMap<String,Object> citem  = CommonUtil.convertV2TIMUserInfoToMap(item);
            clist.add(citem);
        }
        rinfo.put("partialUserList",clist);
        return rinfo;
    }

    public static HashMap<String,Object> convertV2TIMMessageReactionChangeInfoToMap(V2TIMMessageReactionChangeInfo info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("messageID",info.getMessageID());
        List<V2TIMMessageReaction> list = info.getReactionList();
        LinkedList<Object> clist = new LinkedList<Object>();
        for(int i = 0;i<list.size();i++){
            V2TIMMessageReaction item = list.get(i);
            HashMap<String,Object> citem  = CommonUtil.convertV2TIMMessageReactionToMap(item);
            clist.add(citem);
        }
        rinfo.put("reactionList",clist);
        return  rinfo;
    }
    public static V2TIMPermissionGroupInfo HashMapToV2TIMPermissionGroupInfo(HashMap<String,Object> map){
        V2TIMPermissionGroupInfo n_info = new V2TIMPermissionGroupInfo();
        if(map.containsKey("groupID")){
            n_info.setGroupID((String) map.get("groupID"));
        }
        if(map.containsKey("groupPermission")){
            Number num = (Number) map.get("groupPermission");
            n_info.setGroupPermission(Long.valueOf(num.longValue()));
        }
        if(map.containsKey("permissionGroupID")){
            n_info.setPermissionGroupID((String) map.get("permissionGroupID"));
        }
        if(map.containsKey("permissionGroupName")){
            n_info.setPermissionGroupName((String) map.get("permissionGroupName"));
        }
        if(map.containsKey("customData")){
            n_info.setCustomData((String) map.get("customData"));
        }
        return  n_info;
    }
    public static V2TIMGroupInfo HashMapToV2TIMGroupInfo(HashMap<String,Object> info){
        V2TIMGroupInfo n_info = new V2TIMGroupInfo();
        if(info.containsKey("groupID")){
            n_info.setGroupID((String) info.get("groupID"));
        }
        if(info.containsKey("groupType")){

            n_info.setGroupType((String) info.get("groupType"));
        }
        if(info.containsKey("groupName")){

            n_info.setGroupName((String) info.get("groupName"));

        }
        if(info.containsKey("notification")){
            n_info.setNotification((String) info.get("notification"));
        }
        if(info.containsKey("introduction")){
            n_info.setIntroduction((String) info.get("introduction"));
        }
        if(info.containsKey("faceUrl")){
            n_info.setFaceUrl((String) info.get("faceUrl"));
        }
        if(info.containsKey("isAllMuted")){
            n_info.setAllMuted((Boolean) info.get("isAllMuted"));
        }
        if(info.containsKey("isSupportTopic")){
            n_info.setSupportTopic((Boolean) info.get("isSupportTopic"));
        }
        if(info.containsKey("groupAddOpt")){
            n_info.setGroupAddOpt((Integer) info.get("groupAddOpt"));
        }
        if(info.containsKey("approveOpt")){
            n_info.setGroupApproveOpt((Integer) info.get("approveOpt"));
        }
        if(info.containsKey("customInfo")){
            HashMap<String,String> customInfo = (HashMap<String, String>) info.get("customInfo");
            if(customInfo!=null){
                ...
                n_info.setCustomInfo(customInfoByte);
            }
        }
        if(info.containsKey("isEnablePermissionGroup")){
            n_info.setEnablePermissionGroup((Boolean) info.get("isEnablePermissionGroup"));
        }
        if(info.containsKey("defaultPermissions")){
            Number num = (Number) info.get("defaultPermissions");
            n_info.setDefaultPermissions(Long.valueOf(num.longValue()));
        }
        return  n_info;
    }
    public static List<HashMap<String,Object>> convertV2TIMPermissionGroupOperationResultListToMap(List<V2TIMPermissionGroupOperationResult> infos){

        LinkedList<HashMap<String,Object>> rlist = new LinkedList<>();
        for (V2TIMPermissionGroupOperationResult info : infos) {
            rlist.add(CommonUtil.convertV2TIMPermissionGroupOperationResultToMap(info));
        }
        return  rlist;
    }
    public static HashMap<String,Object> convertV2TIMPermissionGroupOperationResultToMap(V2TIMPermissionGroupOperationResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("resultCode",info.getResultCode());
        rinfo.put("resultMessage",info.getResultMessage());
        rinfo.put("permissionGroupID",info.getPermissionGroupID());
        return  rinfo;
    }
    public static List<HashMap<String,Object>> convertV2TIMPermissionGroupInfoResultListToMap(List<V2TIMPermissionGroupInfoResult> infos){

        LinkedList<HashMap<String,Object>> rlist = new LinkedList<>();
        for (V2TIMPermissionGroupInfoResult info : infos) {
            rlist.add(CommonUtil.convertV2TIMPermissionGroupInfoResultToMap(info));
        }
        return  rlist;
    }
    public static HashMap<String,Object> convertV2TIMPermissionGroupInfoResultToMap(V2TIMPermissionGroupInfoResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("resultCode",info.getResultCode());
        rinfo.put("resultMessage",info.getResultMessage());

        rinfo.put("permissionGroupInfo", CommonUtil.convertV2TIMPermissionGroupInfoToMap(info.getPermissionGroupInfo()));
        return  rinfo;
    }

    public static List<HashMap<String,Object>> convertV2TIMPermissionGroupMemberOperationResultListToMap(List<V2TIMPermissionGroupMemberOperationResult> infos){

        LinkedList<HashMap<String,Object>> rlist = new LinkedList<>();
        for (V2TIMPermissionGroupMemberOperationResult info : infos) {
            rlist.add(CommonUtil.convertV2TIMPermissionGroupMemberOperationResultToMap(info));
        }
        return  rlist;
    }
    public static HashMap<String,Object> convertV2TIMPermissionGroupMemberOperationResultToMap(V2TIMPermissionGroupMemberOperationResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("resultCode",info.getResultCode());
        rinfo.put("memberID",info.getMemberID());
        return  rinfo;
    ...
    public static HashMap<String,Object> convertV2TIMPermissionGroupInfoToMap(V2TIMPermissionGroupInfo info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("groupID",info.getGroupID());
        rinfo.put("groupPermission",info.getGroupPermission());
        rinfo.put("permissionGroupID",info.getPermissionGroupID());
        rinfo.put("permissionGroupName",info.getPermissionGroupName());
        rinfo.put("customData",info.getCustomData());
        rinfo.put("memberCount",info.getMemberCount());
        return  rinfo;
    }
    public static HashMap<String,Object> convertV2TIMPermissionGroupMemberInfoResultToMap(V2TIMPermissionGroupMemberInfoResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("nextCursor",info.getNextCursor());
        rinfo.put("memberInfoList",CommonUtil.convertV2TIMGroupMemberFullInfoListToMap(info.getMemberInfoList()));
    ...
    public static HashMap<String,Object> convertV2TIMGroupInfoToMap(V2TIMGroupInfo info){
        ...
        rinfo.put("approveOpt",info.getGroupApproveOpt());
        rinfo.put("isEnablePermissionGroup",info.isEnablePermissionGroup());
        rinfo.put("memberMaxCount",info.getMemberMaxCount());
        rinfo.put("defaultPermissions",info.getDefaultPermissions());
    ...
    public static HashMap<String,Object> convertV2TIMMessageSearchResultToMap(V2TIMMessageSearchResult info){
        ...
        rinfo.put("searchCursor",info.getSearchCursor());
        return  rinfo;
    }
    ...
    public static HashMap<String,Object> convertV2TIMConversationListFilterToMap(V2TIMConversationListFilter info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("conversationType",info.getConversationType());
        rinfo.put("conversationGroup",info.getConversationGroup());
        rinfo.put("markType",info.getMarkType());
        rinfo.put("hasGroupAtInfo",info.getHasGroupAtInfo());
        rinfo.put("hasUnreadCount",info.getHasUnreadCount());
        return rinfo;
    }
    ...
    public static List<HashMap<String,Object>> convertV2TIMTopicOperationResultListToMap(List<V2TIMTopicOperationResult> infos){
        LinkedList<HashMap<String,Object>> rlist = new LinkedList<>();
        for (V2TIMTopicOperationResult info : infos) {
            rlist.add(CommonUtil.convertV2TIMTopicOperationResultToMap(info));
        }
        return  rlist;
    }
    ...
    public static List<HashMap<String,Object>> convertV2TIMTopicPermissionResultListToMap(List<V2TIMTopicPermissionResult> infos){
        LinkedList<HashMap<String,Object>> rlist = new LinkedList<>();
        for (V2TIMTopicPermissionResult info : infos) {
            rlist.add(CommonUtil.convertV2TIMTopicPermissionResultToMap(info));
        }
        return  rlist;
    }

    public static HashMap<String,Object> convertV2TIMTopicPermissionResultToMap(V2TIMTopicPermissionResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("topicID",info.getTopicID());
        rinfo.put("resultCode",info.getResultCode());
        rinfo.put("resultMessage",info.getResultMessage());
        rinfo.put("topicPermission",info.getTopicPermission());
        rinfo.put("groupID",info.getGroupID());
        rinfo.put("germissionGroupID",info.getPermissionGroupID());
        return rinfo;
    }
    ...
    public static HashMap<String,Object>  convertV2TIMMessageExtensionResultToMap(V2TIMMessageExtensionResult status){
        ...
        rinfo.put("resultCode",status.getResultCode());
        rinfo.put("resultInfo",status.getResultInfo());
        rinfo.put("extension",CommonUtil.convertV2TIMMessageExtensionToMap(status.getExtension()));
        return  rinfo;
    }
    ...
    public static HashMap<String,Object>  convertV2TIMMessageReactionResultToMap(V2TIMMessageReactionResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("messageID",info.getMessageID());
        rinfo.put("resultInfo",info.getResultInfo());
        rinfo.put("resultCode",info.getResultCode());
        LinkedList<Map<String,Object>> reactionList = new LinkedList();
        for (int i = 0; i < info.getReactionList().size(); i++) {
            reactionList.add(CommonUtil.convertV2TIMMessageReactionToMap(info.getReactionList().get(i)));
        }
        rinfo.put("reactionList",reactionList);
        return  rinfo;
    }
    public static HashMap<String,Object>  convertV2TIMMessageReactionUserResultToMap(V2TIMMessageReactionUserResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("isFinished",info.getIsFinished());
        rinfo.put("nextSeq",info.getNextSeq());

        LinkedList<Map<String,Object>> userInfoList = new LinkedList();
        for (int i = 0; i < info.getUserInfoList().size(); i++) {
            userInfoList.add(CommonUtil.convertV2TIMUserInfoToMap(info.getUserInfoList().get(i)));
        }
        rinfo.put("userInfoList",userInfoList);
        return  rinfo;
    }
    public static LinkedList<HashMap<String,Object>>  convertV2TIMOfficialAccountInfoResultListToMap(List<V2TIMOfficialAccountInfoResult> infos){
        LinkedList<HashMap<String,Object>> list = new LinkedList<HashMap<String,Object>>();
        for (int i = 0; i < infos.size(); i++) {
            list.add(CommonUtil.convertV2TIMOfficialAccountInfoResultToMap(infos.get(i)));
        }
        return  list;
    }
    public static HashMap<String,Object>  convertV2TIMOfficialAccountInfoResultToMap(V2TIMOfficialAccountInfoResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();

        rinfo.put("officialAccountInfo",CommonUtil.convertV2TIMOfficialAccountInfoToMap(info.getOfficialAccountInfo()));
        rinfo.put("resultInfo",info.getResultInfo());
        rinfo.put("resultCode",info.getResultCode());
        return  rinfo;
    }
    public static HashMap<String,Object>  convertV2TIMFollowOperationResultToMap(V2TIMFollowOperationResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("userID",info.getUserID());
        rinfo.put("resultInfo",info.getResultInfo());
        rinfo.put("resultCode",info.getResultCode());
    ...
    public static HashMap<String,Object>  convertV2TIMOfficialAccountInfoToMap(V2TIMOfficialAccountInfo info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("createTime",info.getCreateTime());
        rinfo.put("customData",info.getCustomData());
        rinfo.put("faceUrl",info.getFaceUrl());
        rinfo.put("officialAccountID",info.getOfficialAccountID());
        rinfo.put("introduction",info.getIntroduction());
        rinfo.put("officialAccountName",info.getOfficialAccountName());
        rinfo.put("organization",info.getOrganization());
        rinfo.put("ownerUserID",info.getOwnerUserID());
        rinfo.put("subscriberCount",info.getSubscriberCount());
        rinfo.put("subscribeTime",info.getSubscribeTime());
        return  rinfo;
    }
    public static LinkedList<HashMap<String,Object>>   convertV2TIMFollowOperationResultListToMap(List<V2TIMFollowOperationResult> infos){
        LinkedList<HashMap<String,Object>> list = new LinkedList<HashMap<String,Object>>();
        for (int i = 0; i < infos.size(); i++) {
            list.add(CommonUtil.convertV2TIMFollowOperationResultToMap(infos.get(i)));
        }
        return  list;
    }
    public static LinkedList<HashMap<String,Object>>   convertV2TIMUserFullInfoListToMap(List<V2TIMUserFullInfo> infos){
        LinkedList<HashMap<String,Object>> list = new LinkedList<HashMap<String,Object>>();
        for (int i = 0; i < infos.size(); i++) {
            list.add(CommonUtil.convertV2TIMUserFullInfoToMap(infos.get(i)));
        }
        return  list;
    }
    public static HashMap<String,Object>  convertV2TIMUserInfoResultToMap(V2TIMUserInfoResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("nextCursor",info.getNextCursor());
        rinfo.put("userFullInfoList",CommonUtil.convertV2TIMUserFullInfoListToMap(info.getUserFullInfoList()));
        return  rinfo;
    }
    public static LinkedList<HashMap<String,Object>>   convertV2TIMFollowInfoListToMap(List<V2TIMFollowInfo> infos){
        LinkedList<HashMap<String,Object>> list = new LinkedList<HashMap<String,Object>>();
        for (int i = 0; i < infos.size(); i++) {
            list.add(CommonUtil.convertV2TIMFollowInfoToMap(infos.get(i)));
        }
        return  list;
    }
    public static HashMap<String,Object>  convertV2TIMFollowInfoToMap(V2TIMFollowInfo info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();

        rinfo.put("resultInfo",info.getResultInfo());
        rinfo.put("followersCount",info.getFollowersCount());
        rinfo.put("userID",info.getUserID());
        rinfo.put("resultCode",info.getResultCode());
        rinfo.put("mutualFollowersCount",info.getMutualFollowersCount());
        rinfo.put("followingCount",info.getFollowingCount());
        return  rinfo;
    }
    public static LinkedList<HashMap<String,Object>>   convertV2TIMFollowTypeCheckResultListToMap(List<V2TIMFollowTypeCheckResult> infos){
        LinkedList<HashMap<String,Object>> list = new LinkedList<HashMap<String,Object>>();
        for (int i = 0; i < infos.size(); i++) {
            list.add(CommonUtil.convertV2TIMFollowTypeCheckResultToMap(infos.get(i)));
        }
        return  list;
    }
    public static HashMap<String,Object>  convertV2TIMFollowTypeCheckResultToMap(V2TIMFollowTypeCheckResult info){
        HashMap<String,Object> rinfo = new HashMap<String,Object>();
        rinfo.put("followType",info.getFollowType());
        rinfo.put("resultInfo",info.getResultInfo());
        rinfo.put("userID",info.getUserID());
        rinfo.put("resultCode",info.getResultCode());
        return  rinfo;
