...
public class GroupManager {
    ...
    public void createGroup(MethodCall methodCall, final MethodChannel.Result result){
        ...
        if(CommonUtil.getParam(methodCall,result,"approveOpt")!=null){
            int approveOpt = CommonUtil.getParam(methodCall,result,"approveOpt");
            info.setGroupApproveOpt(approveOpt);
        }
        ...
        if(CommonUtil.getParam(methodCall,result,"isEnablePermissionGroup")!=null){
            boolean isEnablePermissionGroup = CommonUtil.getParam(methodCall,result,"isEnablePermissionGroup");
            info.setEnablePermissionGroup(isEnablePermissionGroup);
        }
        if(CommonUtil.getParam(methodCall,result,"defaultPermissions")!=null){
            Number defaultPermissions = CommonUtil.getParam(methodCall,result,"defaultPermissions");
            info.setDefaultPermissions(Long.valueOf(defaultPermissions.longValue()));
        }
    ...
    public void createTopicInCommunity(MethodCall methodCall, final MethodChannel.Result result) {
        ...
        if(topicInfo.get("defaultPermissions")!=null){
            Number defaultPermissions = (Number) topicInfo.get("defaultPermissions");
            info.setDefaultPermissions(Long.valueOf(defaultPermissions.longValue()));
        }
                ...
                public void onSuccess(List<V2TIMTopicInfoResult> v2TIMTopicInfoResults) {
                    if(v2TIMTopicInfoResults.size()==1){
                        ...
                        if(res.getErrorCode() == 0){
                            ...
                            if(topicInfo.get("defaultPermissions")!=null){
                                Number defaultPermissions =  (Number)topicInfo.get("defaultPermissions");
                                topicInfo_native.setDefaultPermissions(Long.valueOf(defaultPermissions.longValue()));
                            }
...
//        });
    ...
    public void kickGroupMember(MethodCall methodCall,final MethodChannel.Result result){
        ...
        int duration = CommonUtil.getParam(methodCall,result,"duration");
        V2TIMManager.getGroupManager().kickGroupMember(groupID, memberList, reason,duration, new V2TIMValueCallback<List<V2TIMGroupMemberOperationResult>>() {
            @Override
            public void onError(int i, String s) {
                CommonUtil.returnError(result,i,s);
            }
    ...
    public void acceptGroupApplication(MethodCall methodCall,final MethodChannel.Result result){
        ...
        final String toUser= CommonUtil.getParam(methodCall,result,"toUser");
         long addTime = 0;
        try{
            addTime = methodCall.argument("addTime");
        }catch (Exception e){
        ...
        V2TIMManager.getGroupManager().getGroupApplicationList(new V2TIMValueCallback<V2TIMGroupApplicationResult>() {
            ...
            public void onSuccess(V2TIMGroupApplicationResult v2TIMGroupApplicationResult) {
                ...
                for (int i=0;i<v2TIMGroupApplicationResult.getGroupApplicationList().size();i++){
                    ...
                    if(application.getGroupID().equals(groupID) && application.getFromUser().equals(fromUser) && application.getToUser().equals(toUser) && application.getAddTime() == addTimeCopy && application.getApplicationType()==type){
                        getapp = application;
                        break;
                    }
                }
    ...
    public void setGroupCounters(MethodCall methodCall,final MethodChannel.Result result){
         String groupID =  CommonUtil.getParam(methodCall,result,"groupID");
        HashMap<String, Integer> counters =   CommonUtil.getParam(methodCall,result,"counters");
        HashMap<String,Long> countersLong = new HashMap<>();
        ...
        while (it.hasNext()) {
            ...
            long value_l = (long) 0;
            try {
               value_l =  Long.valueOf(value.toString());
            }catch (Exception e){

            }
                         countersLong.put((String) key,value_l);

                     }

        V2TIMManager.getGroupManager().setGroupCounters(groupID, countersLong, new V2TIMValueCallback<Map<String, Long>>() {
            @Override
            public void onSuccess(Map<String, Long> stringLongMap) {
                CommonUtil.returnSuccess(result,stringLongMap);
            }
    ...
    public void getGroupCounters(MethodCall methodCall,final MethodChannel.Result result){
        String groupID =  CommonUtil.getParam(methodCall,result,"groupID");
        List<String> keys = CommonUtil.getParam(methodCall,result,"keys");
        V2TIMManager.getGroupManager().getGroupCounters(groupID, keys, new V2TIMValueCallback<Map<String, Long>>() {
            @Override
            public void onSuccess(Map<String, Long> stringLongMap) {
                CommonUtil.returnSuccess(result,stringLongMap);
            }
    ...
    public void increaseGroupCounter(MethodCall methodCall,final MethodChannel.Result result){
        String groupID =  CommonUtil.getParam(methodCall,result,"groupID");
        String key =  CommonUtil.getParam(methodCall,result,"key");
        int value =  CommonUtil.getParam(methodCall,result,"value");
        V2TIMManager.getGroupManager().increaseGroupCounter(groupID, key, value, new V2TIMValueCallback<Map<String, Long>>() {
            @Override
            public void onSuccess(Map<String, Long> stringLongMap) {
                CommonUtil.returnSuccess(result,stringLongMap);
            }
    ...
    public void decreaseGroupCounter(MethodCall methodCall,final MethodChannel.Result result){
        String groupID =  CommonUtil.getParam(methodCall,result,"groupID");
        String key =  CommonUtil.getParam(methodCall,result,"key");
        int value =  CommonUtil.getParam(methodCall,result,"value");
        V2TIMManager.getGroupManager().decreaseGroupCounter(groupID, key, value, new V2TIMValueCallback<Map<String, Long>>() {
            @Override
            public void onSuccess(Map<String, Long> stringLongMap) {
                CommonUtil.returnSuccess(result,stringLongMap);
            }
    ...
    public void markGroupMemberList(MethodCall methodCall,final MethodChannel.Result result){
        String groupID =  CommonUtil.getParam(methodCall,result,"groupID");
        List<String> memberIDList = CommonUtil.getParam(methodCall,result,"memberIDList");
        ...
        V2TIMManager.getGroupManager().markGroupMemberList(groupID, memberIDList, markType, enableMark, new V2TIMCallback() {
            @Override
            public void onSuccess() {
                CommonUtil.returnSuccess(result,null);
            }
